...
class TaxPrinter:
    def __init__(self):
        ...
        self.vars = {
            ...
            'pad': 5,
            'var-date': StrVar(),
            'var-filename': StrVar(value='opis'),
            'var-point-text': StrVar(value='<b>Punkt kosztorysu <p></b> „<o>”'),
            'var-cash': BoolVar(),
            'var-addons': BoolVar(),
            'var-cash-text': StrVar(value='Kwota <b>............... zł</b>, '),
            'var-addons-text': StrVar(value=' - <br>'),
            'opening-text': 'Sprawdzono pod względem formalnym, merytorycznym i rachunkowym. AKCEPTUJĘ DO ZAPŁATY.\nPodatek i składki zostały odprowadzone do właściwych urzędów w ustawowym terminie, niewykraczającym poza końcowy termin realizacji zadania określony w umowie.\nZapłacono gotówką / przelewem dnia ……………………\nUjęto w księgach rachunkowych na wyodrębnionym koncie ……………………\n',
            'style': Style(),
            'tags': ('b', 'i', 'u', 's')
            }
        self.elem = {
    ...
    def __check(f):
        def wrapper(self, *args):
            if f(self, *args):
                self.elem.get('btn-print').config(state='normal' if self.elem.get('tree-selected').get_children() else 'disabled')
        return wrapper
    ...
    def __get_date(self, date, dates):
        ...
        date, dates = self.__str2date(date), list(map(self.__str2date, dates))
